...
print(model_name)
model = load_model(
    model_name, custom_objects={"iou_score": iou_score, "f1-score": f1_score}
)
# model.summary()
imagepaths = sorted(
    glob(os.path.join(train_path, train_img_folder, "sperm", "image_*"))
)
labelpaths = sorted(
    glob(os.path.join(train_path, train_label_folder, "sperm", "image_*"))
)
...
for imagepath, labelpath in zip(imagepaths, labelpaths):
    ...
    bfl = binary_focal_loss(
        K.constant(y_true_.astype("float64")), K.constant(y_pred_.astype("float64"))
    ).numpy()
    binary_focal_losses.append(bfl)
    print(bfl)

    binary_focal_loss3 = BinaryFocalLoss(gamma=3.0)
    bfl3 = binary_focal_loss3(
        K.constant(y_true_.astype("float64")), K.constant(y_pred_.astype("float64"))
    ).numpy()
...
pd.DataFrame(
    {
        ...
        "white_pixel_count": white_pixel_counts,
    }
).to_csv("model_check_%s.csv" % model_name, index=False)
